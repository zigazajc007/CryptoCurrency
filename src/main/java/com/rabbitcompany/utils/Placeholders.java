...
public class Placeholders extends PlaceholderExpansion {
	...
	public String onRequest(OfflinePlayer offlinePlayer, String identifier) {
		...
		if (identi.length == 3 && identi[0].equals("balance") && identi[1].equals("fiat"))
			return API.moneyFormatter.format(API.getTotalAssets(identi[2]));
		...
		if (identi.length == 3 && identi[1].equals("price") && Number.isNumeric(identi[2]))
			return API.getCryptoPriceFormatted(identi[0], Double.parseDouble(identi[2]));
		//%cryptocurrency_<crypto>_balance_<player>%
		if (identi.length == 3 && identi[1].equals("balance") && !identi[2].equals("fiat"))
			return API.getBalanceFormatted(identi[2], identi[0]);
		//%cryptocurrency_<crypto>_balance_fiat_<player>%
		if (identi.length == 4 && identi[1].equals("balance") && identi[2].equals("fiat"))
			return API.moneyFormatter.format(API.getCryptoPrice(identi[0], API.getBalance(identi[3], identi[0])));
		//%cryptocurrency_<crypto>_supply%
		if (identi.length == 2 && identi[1].equals("supply"))
			return API.getFormatter(identi[0]).format(API.getCryptoSupply(identi[0]));
		//%cryptocurrency_<crypto>_maxSupply%
		if (identi.length == 2 && identi[1].equals("maxSupply"))
			return API.getFormatter(identi[0]).format(API.getCryptoMaxSupply(identi[0]));
		//%cryptocurrency_<crypto>_marketCap%
		if (identi.length == 2 && identi[1].equals("marketCap"))
			return API.moneyFormatter.format(API.getCryptoMarketCap(identi[0]));
		...
		if (identi.length == 5 && identi[1].equals("fast") && identi[2].equals("change") && identi[3].equals("percent") && identi[4].equals("abs")) {
			double previousCryptoPrice = Settings.cryptos.get(identi[0]).history.getDouble(LocalDate.now() + ".avg", Settings.cryptos.get(identi[0]).price);
			double cryptoPrice = Settings.cryptos.get(identi[0]).price;
			return String.valueOf(Number.roundDouble(Math.abs(previousCryptoPrice - cryptoPrice) / ((previousCryptoPrice + cryptoPrice) / 2.0) * 100, 2));
		}
		...
		if (identi.length == 5 && identi[1].equals("daily") && identi[2].equals("change") && identi[3].equals("percent") && identi[4].equals("abs")) {
			...
			double cryptoPrice = Settings.cryptos.get(identi[0]).price;
			return String.valueOf(Number.roundDouble(Math.abs(dailyCryptoPrice - cryptoPrice) / ((dailyCryptoPrice + cryptoPrice) / 2.0) * 100, 2));
		}
		...
		if (identi.length == 5 && identi[1].equals("weekly") && identi[2].equals("change") && identi[3].equals("percent") && identi[4].equals("abs")) {
			...
			double cryptoPrice = Settings.cryptos.get(identi[0]).price;
			return String.valueOf(Number.roundDouble(Math.abs(weeklyCryptoPrice - cryptoPrice) / ((weeklyCryptoPrice + cryptoPrice) / 2.0) * 100, 2));
		}
		...
		if (identi.length == 5 && identi[1].equals("monthly") && identi[2].equals("change") && identi[3].equals("percent") && identi[4].equals("abs")) {
			...
			double cryptoPrice = Settings.cryptos.get(identi[0]).price;
			return String.valueOf(Number.roundDouble(Math.abs(monthlyCryptoPrice - cryptoPrice) / ((monthlyCryptoPrice + cryptoPrice) / 2.0) * 100, 2));
		}
		...
		if (identi.length == 3 && identi[1].equals("balance") && identi[2].equals("fiat"))
			return API.moneyFormatter.format(API.getCryptoPrice(identi[0], API.getBalance(player.getName(), identi[0])));
		...
		if (identi.length == 4 && identi[1].equals("change") && identi[2].equals("percent") && identi[3].equals("player")) {
			...
			double yesterdayPrice = yesterdayCryptoPrice * balance;
			double price = API.getCryptoPrice(identi[0], balance);
			return String.valueOf(Number.roundDouble(Math.abs(yesterdayPrice - price) / ((yesterdayCryptoPrice + price) / 2.0) * 100, 2));
		}
		return "";
	}
}
